...
5. **For the active workstream:** `docs/superpowers/handovers/HANDOVER-2026-06-14-flows-replatform-plan.md`, then the design `docs/superpowers/specs/2026-06-14-flows-dialog-replatform-design.md` and the plan `docs/superpowers/plans/2026-06-14-flows-dialog-replatform.md`

## Immediate Next Steps

Per the D-15 audit; brainstorm → spec → plan done 2026-06-14. **Next action: execute the plan** (`docs/superpowers/plans/2026-06-14-flows-dialog-replatform.md`) via subagent-driven development on branch `feature/d15-flows-replatform`. Code-only tasks (P0.1 prices, P0.2 per-role eval endpoints, T1 flag, then T2/T3 nodes+handlers) can run without providers; **P0.3 model A/B and S1 compat spike need real provider keys + `uv sync --extra call` + network (owner-run).** Roadmap:
