...
| Small conversation LLM drifts out of caller role mid-call | Agent acts as the callee's receptionist; task fails, callee confused | Explicit role block + language-aware few-shot (PR #7); offline A/B 3/3; eval harness `doctor/role_drift_probe` 3/3 tool-enabled multi-turn (2026-06-12); intermittent marker slip still seen in `booking_basic` | Mitigated in code, measurable offline; awaiting live confirmation |
...
| haiku omits `end_call`/`propose_summary` at conversation end | Calls trail off without clean termination/summary; runs stuck "running" | Investigate prompt/termination nudge vs model floor (D-11); eval success axis now measures it | Open (found 2026-06-12 by eval harness) |
